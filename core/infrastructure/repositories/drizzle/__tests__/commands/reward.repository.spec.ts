...
describe('RewardRepositoryDrizzle', () => {
  ...
  describe('create', () => {
    it('should insert a new reward and return the mapped domain entity', async () => {
      ...
      expect(valuesMock).toHaveBeenCalledWith(
        sampleRewardDomain.toPersistence(),
      );
    ...
    it('should propagate DB errors during creation', async () => {
      ...
      await expect(repository.create(sampleRewardDomain)).rejects.toThrow(
        'Insert Failed',
      );
    });
  });
  ...
  describe('update', () => {
    it('should update the reward and return the mapped result', async () => {
      ...
      const { id: _id, ...dataWithoutId } = sampleRewardDomain.toPersistence();
    ...
    it('should propagate DB errors during update', async () => {
      ...
      await expect(repository.update(sampleRewardDomain)).rejects.toThrow(
        'Update Failed',
      );
    });
  });
