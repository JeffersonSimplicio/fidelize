...
describe('CustomerRepositoryDrizzle', () => {
  ...
  describe('create', () => {
    it('should insert a new customer and return the mapped domain entity', async () => {
      ...
      expect(valuesMock).toHaveBeenCalledWith(
        sampleCustomerDomain.toPersistence(),
      );
    ...
    it('should propagate DB errors during creation', async () => {
      const returningMock = jest
        .fn()
        .mockRejectedValue(new Error('DB Constraint Error'));
      const valuesMock = jest
        .fn()
        .mockReturnValue({ returning: returningMock });
      mockDb.insert = jest.fn().mockReturnValue({ values: valuesMock });

      await expect(repository.create(sampleCustomerDomain)).rejects.toThrow(
        'DB Constraint Error',
      );
    });
  });
  ...
  describe('update', () => {
    it('should update customer data and return the mapped result', async () => {
      const updatedSelect: CustomerSelect = {
        ...sampleCustomerSelect,
        points: 200,
      };
      const updatedDomain: Customer = {
        ...sampleCustomerDomain,
        points: 200,
      } as any;
    ...
    it('should propagate DB errors during update', async () => {
      ...
      await expect(repository.update(sampleCustomerDomain)).rejects.toThrow(
        'Update Failed',
      );
    });
  });
  ...
  describe('delete', () => {
    ...
    it('should propagate DB errors during deletion', async () => {
      const whereMock = jest
        .fn()
        .mockRejectedValue(new Error('Delete Constraint Error'));
      mockDb.delete = jest.fn().mockReturnValue({ where: whereMock });
