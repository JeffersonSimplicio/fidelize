...
describe("ListRewardsRedeemedByCustomerUseCase", () => {
  ...
  it("should return mapped rewards redeemed by customer", async () => {
    ...
    const rewardA = new Reward({ name: "R-A", description: "a", pointsRequired: 10, isActive: RewardStatus.Active });
    rewardA.setId(10);
    const rewardB = new Reward({ name: "R-B", description: "b", pointsRequired: 20, isActive: RewardStatus.Active });
    rewardB.setId(11);
    ...
    customerRewardQueryRepo.findRewardsRedeemedByCustomer.mockResolvedValue([itemA, itemB]);
    ...
    expect(customerRewardQueryRepo.findRewardsRedeemedByCustomer).toHaveBeenCalledWith(customer.id);
  ...
  it("should propagate error when findRewardsRedeemedByCustomer throws", async () => {
    ...
    customerRepo.getById.mockResolvedValue(customer);
    customerRewardQueryRepo.findRewardsRedeemedByCustomer.mockRejectedValue(new Error("Query failure"));
  ...
  it("should propagate error when rewardToDtoMapper.map throws for an item", async () => {
    ...
    const reward = new Reward({ name: "Bad", description: "bad", pointsRequired: 1, isActive: RewardStatus.Active });
    reward.setId(50);

    const redeemedAt = new Date();
    customerRepo.getById.mockResolvedValue(customer);
    customerRewardQueryRepo.findRewardsRedeemedByCustomer.mockResolvedValue([new CustomerRedeemedReward(reward, redeemedAt)]);
  ...
  it("should call getById before findRewardsRedeemedByCustomer (order of operations)", async () => {
    ...
    customerRepo.getById.mockImplementation(async () => { order.push("getById"); return customer; });
    customerRewardQueryRepo.findRewardsRedeemedByCustomer.mockImplementation(async () => { order.push("findRedeemed"); return []; });
  ...
  it("should not call unrelated repository methods", async () => {
    ...
    const reward = new Reward({ name: "Only", description: "o", pointsRequired: 1, isActive: RewardStatus.Active });
    reward.setId(60);

    customerRepo.getById.mockResolvedValue(customer);
    customerRewardQueryRepo.findRewardsRedeemedByCustomer.mockResolvedValue([new CustomerRedeemedReward(reward, new Date())]);
    ...
    expect(customerRewardQueryRepo.findRewardsRedeemedByCustomer).toHaveBeenCalledTimes(1);
    ...
    expect(customerRewardQueryRepo.findCustomersEligibleToRedeemReward)
      .not.toHaveBeenCalled();
