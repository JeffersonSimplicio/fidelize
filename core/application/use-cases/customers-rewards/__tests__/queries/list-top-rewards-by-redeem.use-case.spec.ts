...
describe("ListTopRewardsByRedeemUseCase", () => {
  ...
  it("should return mapped top rewards (happy path)", async () => {
    const reward1 = new Reward({ name: "Reward1", description: "Desc1", pointsRequired: 10, isActive: RewardStatus.Active });
    reward1.setId(1);
    const reward2 = new Reward({ name: "Reward2", description: "Desc2", pointsRequired: 20, isActive: RewardStatus.Active });
    reward2.setId(2);

    const topReward1 = new TopReward(reward1, 5);
    const topReward2 = new TopReward(reward2, 3);

    customerRewardQueryRepo.findTopRewardsByRedeem.mockResolvedValue([topReward1, topReward2]);

    const dto1: RewardDto = { id: 1, name: "Reward1", pointsRequired: 10, description: "Desc1", isActive: true, createdAt: reward1.createdAt.toISOString() };
    const dto2: RewardDto = { id: 2, name: "Reward2", pointsRequired: 20, description: "Desc2", isActive: true, createdAt: reward2.createdAt.toISOString() };

    rewardToDtoMapper.map.mockImplementation(r => r.id === 1 ? dto1 : dto2);

    const result = await useCase.execute(2);

    expect(customerRewardQueryRepo.findTopRewardsByRedeem).toHaveBeenCalledWith(2);
  ...
  it("should default limit to 3 if not provided", async () => {
    ...
    expect(customerRewardQueryRepo.findTopRewardsByRedeem).toHaveBeenCalledWith(3);
    expect(result).toEqual([]);
  });
  ...
  it("should use minimum limit of 1 if a non-positive limit is provided", async () => {
    ...
    expect(customerRewardQueryRepo.findTopRewardsByRedeem).toHaveBeenCalledWith(1);
    await useCase.execute(-5);
    expect(customerRewardQueryRepo.findTopRewardsByRedeem).toHaveBeenCalledWith(1);
  });

  it("should propagate error when findTopRewardsByRedeem throws", async () => {
    customerRewardQueryRepo.findTopRewardsByRedeem.mockRejectedValue(new Error("Query failed"));
  ...
  it("should propagate error when rewardToDtoMapper.map throws", async () => {
    const reward = new Reward({ name: "Fail", description: "Fail", pointsRequired: 1, isActive: RewardStatus.Active });
    reward.setId(10);

    customerRewardQueryRepo.findTopRewardsByRedeem.mockResolvedValue([new TopReward(reward, 7)]);
  ...
  it("should not call unrelated repository methods", async () => {
    const reward = new Reward({ name: "R", description: "D", pointsRequired: 1 });
    reward.setId(1);
    customerRewardQueryRepo.findTopRewardsByRedeem.mockResolvedValue([new TopReward(reward, 2)]);
    rewardToDtoMapper.map.mockReturnValue({ id: 1, name: "R", pointsRequired: 1, description: "D", isActive: true, createdAt: reward.createdAt.toISOString() });
