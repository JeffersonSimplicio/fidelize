...
describe("ListCustomersWhoRedeemedRewardUseCase", () => {
  ...
  it("should return mapped CustomerRewardRedemptionDto array (happy path)", async () => {
    const reward = new Reward({ name: "Prize", description: "d", pointsRequired: 5, isActive: RewardStatus.Active });
    reward.setId(7);
    ...
    customerRewardQueryRepo.findCustomersWhoRedeemedReward.mockResolvedValue([item1, item2]);
    ...
    expect(customerRewardQueryRepo.findCustomersWhoRedeemedReward).toHaveBeenCalledWith(reward.id);
  ...
  it("should return an empty array when no customers redeemed the reward", async () => {
    const reward = new Reward({ name: "None", description: "d", pointsRequired: 1, isActive: RewardStatus.Active });
    reward.setId(8);
  ...
  it("should propagate error when findCustomersWhoRedeemedReward throws", async () => {
    const reward = new Reward({ name: "Err", description: "d", pointsRequired: 1, isActive: RewardStatus.Active });
    reward.setId(9);

    rewardRepo.getById.mockResolvedValue(reward);
    customerRewardQueryRepo.findCustomersWhoRedeemedReward.mockRejectedValue(new Error("Query failed"));
  ...
  it("should propagate error when customerDtoMapper.map throws for an item", async () => {
    const reward = new Reward({ name: "MapErr", description: "d", pointsRequired: 1, isActive: RewardStatus.Active });
    reward.setId(10);
    ...
    rewardRepo.getById.mockResolvedValue(reward);
    customerRewardQueryRepo.findCustomersWhoRedeemedReward.mockResolvedValue([new CustomerRewardRedemption(cust, redeemedAt)]);
    customerDtoMapper.map.mockImplementation(() => { throw new Error("Mapping failed"); });
  ...
  it("should call getById before findCustomersWhoRedeemedReward (order of operations)", async () => {
    const order: string[] = [];
    const reward = new Reward({ name: "OrderTest", description: "d", pointsRequired: 1, isActive: RewardStatus.Active });
    reward.setId(11);

    rewardRepo.getById.mockImplementation(async () => { order.push("getById"); return reward; });
    customerRewardQueryRepo.findCustomersWhoRedeemedReward.mockImplementation(async () => { order.push("findWhoRedeemed"); return []; });
  ...
  it("should not call unrelated repository methods", async () => {
    const reward = new Reward({ name: "NoExtra", description: "d", pointsRequired: 1, isActive: RewardStatus.Active });
    reward.setId(12);
    ...
    expect(customerRewardQueryRepo.findCustomersWhoRedeemedReward).toHaveBeenCalledTimes(1);
