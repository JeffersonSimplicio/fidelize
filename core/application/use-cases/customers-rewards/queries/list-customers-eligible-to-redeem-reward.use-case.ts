...
export class ListEligibleCustomersForRewardUseCase
  implements ListCustomersEligibleToRedeemReward
{
  ...
  async execute(rewardId: number): Promise<CustomerDto[]> {
    ...
    const eligibleCustomers =
      await this.customerRewardQueryRepo.findCustomersEligibleToRedeemReward(
        rewardId,
      );
