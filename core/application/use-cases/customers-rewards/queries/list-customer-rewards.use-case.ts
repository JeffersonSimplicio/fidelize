...
export class ListCustomerRewardsUseCase implements ListCustomerRewards {
  ...
  private readonly customerRewardToDtoMapper: Mapper<CustomerReward, CustomerRewardDto>;
  ...
  async execute(): Promise<CustomerRewardDto[]> {
    ...
    return customerRewards.map(reward => this.customerRewardToDtoMapper.map(reward));
  }
}